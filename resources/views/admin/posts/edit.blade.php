...
@section('title', 'Edit Travel Post - Go-Blog Admin')

@section('content')
<form action="{{ route('admin.posts.update', $post->id) }}" method="POST" class="max-w-6xl mx-auto pb-12">
    @csrf
    @method('PUT')
    ...
    <div class="flex items-center justify-between mb-8">
        <nav class="flex items-center gap-2 text-xs font-medium">
            ...
            <span class="text-slate-800">Edit Travel Post</span>
        </nav>
                ...
                <div class="p-8">
                    <div class="prose max-w-none">
                        <textarea name="body" id="body" rows="15" 
                                  ...
                                  placeholder="Tucked away beneath the emerald canopy of Chiang Mai's northern provinces lies a secret that few travelers ever witness..." required>{{ old('body', $post->body) }}</textarea>
                    </div>
                    ...
                    <div class="mt-8 rounded-2xl overflow-hidden bg-slate-100 aspect-video flex flex-col items-center justify-center border-2 border-dashed border-slate-200 group cursor-pointer hover:bg-slate-50 transition-colors">
                        ...
                        <p class="text-sm font-bold text-slate-400 mt-4 group-hover:text-slate-600">Change featured image</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                ...
                <div class="space-y-2">
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors has-[:checked]:border-admin-blue has-[:checked]:bg-blue-50/50 group">
                        <input type="radio" name="category" value="Adventure" class="w-4 h-4 text-admin-blue border-slate-200 focus:ring-admin-blue/20" {{ $post->category === 'Adventure' ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors has-[:checked]:border-admin-blue has-[:checked]:bg-blue-50/50 group">
                        <input type="radio" name="category" value="Luxury Travel" class="w-4 h-4 text-admin-blue border-slate-200 focus:ring-admin-blue/20" {{ $post->category === 'Luxury Travel' ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors has-[:checked]:border-admin-blue has-[:checked]:bg-blue-50/50 group">
                        <input type="radio" name="category" value="Local Cuisine" class="w-4 h-4 text-admin-blue border-slate-200 focus:ring-admin-blue/20" {{ $post->category === 'Local Cuisine' ? 'checked' : '' }}>
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm space-y-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="text" placeholder="Hidden Waterfalls Northern Thailand | Wanderlust" class="w-full bg-slate-50 border-none rounded-xl px-4 py-2.5 text-xs font-medium text-slate-700 placeholder:text-slate-300 focus:ring-2 focus:ring-admin-blue/20" value="{{ $post->title }}">
                    </div>
                    <div>
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Description</label>
                        <textarea placeholder="Discover the secret cascades of Chiang Mai that tourists miss..." rows="3" class="w-full bg-slate-50 border-none rounded-xl px-4 py-2.5 text-xs font-medium text-slate-700 placeholder:text-slate-300 focus:ring-2 focus:ring-admin-blue/20 resize-none">{{ \Illuminate\Support\Str::limit($post->body, 160) }}</textarea>
                    </div>
                </div>
            </div>
